...
//
// system-level testbench for the decrypt pipeline (stage d3).
// all dut-facing signals driven from clocked always blocks.
//
// test 1: decrypt nist tc14 (default key/iv)
//   44 bytes in (12 iv + 16 ct + 16 tag) -> 16 bytes pt out
// test 2: decrypt nist tc15 (loaded key)
//   92 bytes in (12 iv + 64 ct + 16 tag) -> 64 bytes pt out
// test 3: decrypt with corrupted tag (auth fail)
//   44 bytes in -> 16 bytes pt out, auth_fail = 1
//
...
module tb_aes_gcm_256_axis_decrypt_top;
    ...
    initial begin
        ...
        // encrypted packet: iv(0) + ct(cea7...18) + tag(d0d1...19) = 44 bytes
        // expected pt: 16 bytes of 0x00
        ...
        // test 2: decrypt nist tc15 (loaded key)
        // key = feffe992...08, iv from packet = cafebabe...888
        // 92 bytes in -> 64 bytes pt out
        $display("");
        ...
        // fill encrypted packet (92 bytes)
        ...
        // test 3: decrypt with corrupted tag (auth fail)
        // same as tc14 but tag all-ff -> auth_fail = 1
        $display("");
        $display("test 3: decrypt with corrupted tag");
        ...
        fill_pkt_96(0, 96'h000000000000000000000000);
        fill_pkt_128(12, 128'hcea7403d4d606b6e074ec5d3baf39d18);
        fill_pkt_128(28, 128'hFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF); // corrupted
        rxLen = 16'd44;
        ...
        totalTests = totalTests + 1;
        if (txCount == 16) begin
            $display("    PASS: %0d bytes output (pt still produced)", txCount);
            passCount = passCount + 1;
        end else begin
            $display("    FAIL: %0d bytes output (expected 16)", txCount);
            failCount = failCount + 1;
        ...
        totalTests = totalTests + 1;
        if (authFail) begin
            $display("    PASS: auth_fail asserted (tag corrupted)");
            passCount = passCount + 1;
        end else begin
