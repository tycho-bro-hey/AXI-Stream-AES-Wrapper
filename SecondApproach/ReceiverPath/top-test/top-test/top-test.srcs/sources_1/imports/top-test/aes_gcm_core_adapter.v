            ...
            case (state)
                ...
                STATE_WAIT_DONE: begin
                    if (coreDone) begin
                        tagOut <= coreTag;
                        tagValid <= 1'b1;
                        // compare computed tag against expected tag from port
                        // (core's internal tag_match/auth_fail use tag_in_reg
                        // which was latched on start -- too early for decrypt
                        // pipeline where tag arrives after ct bytes. tag-in port
                        // is valid by now since parser finishes first.)
                        if (modeReg) begin
                            tagMatch <= (coreTag == tagIn);
                            authFail <= (coreTag != tagIn);
                        end
                        else begin
                            tagMatch <= 1'b0;
                            authFail <= 1'b0;
